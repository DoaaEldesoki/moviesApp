...
const favoritesReducer = (favorites = { favoriteFilms: [], totalCount: 0 }, action) => {
    switch (action.type) {
        case ADD_FAVORITES:
        ...
        case DELETE_FAVORITES:
            if (favorites.totalCount > 0) {
                return { ...favorites, favoriteFilms: [...favorites.favoriteFilms.filter((item) => item.id !== action.payload)], totalCount: [...favorites.favoriteFilms].length - 1 };
            } else {
                return favorites;
            }
        default:
